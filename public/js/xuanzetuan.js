...
var ratio;
 ...
 console.log("58 - window.devicePixelRatio:", window.devicePixelRatio)
console.log("59 - isWXPC:", isWXPC)

if (isWXPC) {//window.devicePixelRatio < 2
...
function preLoadMod() {
	...
	loader.load('model/personal.FBX', function(object) {

			if (object) {
				// console.log("人物模型预载完毕")
			};
...
$.ajax({
	...
	success: function(data) {
		...
		if (data.code == 1) {
			//口罩列表
			getGoodsList = data.data.goods_list;
			...
			for (var i in getPicList) {
				...
				if (i == getPicList.length - 1) {
					...
					document.getElementById("pic_img" + getPicList[i].id).onload = function() {
						// console.log("pic_img" + getPicList[i].id + "图片加载完成")
						preLoadMod()
					}
				}
			}
			...
			//颜色列表
			getColorList = data.data.color_list;
			//更换口罩颜色
			// const canvas1 = document.createElement("canvas")
			// const ctx = canvas1.getContext("2d")
			// canvas1.width = canvas.width;
...
//收藏
function add_collect(file) {


	var fd = new FormData()
	fd.append('letter_img', file)
	fd.append('goods_id', getId)
...
function uploadApi() {
	...
	image.onload = function() {
		...
		var file = base64ImgtoFile(newImageData);

		var fd = new FormData()
		fd.append('file', file)
		$.ajax({

			url: apiUrl + "api/file/CustomImage",
			data: fd,
			contentType: false,
			//取消帮我们格式化数据，是什么就是什么
			processData: false,
			...
			success: function(data, status) {
				...
				if (data.code == 1) {
					...
					if (data.data.url) {
						fav_img = data.data.url;
						that.add_collect(data.data.url);
						return true;
					}
					alert('收藏失败');
					return false;
...
$(".view3d").click(function() {
	...
	if (yl_img != '') {
		...
		var pageUrl = "index.html?id=" + getId + "&img=" + yl_img + "&yimg=" + ys_img + '&token=' + token;
		// $('#newPage').load("index.html?id=" + getId + "&img=" + data.data.url);
		document.getElementById("newPage").innerHTML = '<object type="text/html" data="' + pageUrl +
	...
	image.onload = function() {
		...
		bgImg.onload = () => {
			...
			img.onload = () => {
				...
				// if(canvas.getActiveObject()){
				// 	canvasPath = canvas.getActiveObject().clipPath;
					
					
				// }

				// canvas.getActiveObject().clipPath = null;

				let el = canvas.getObjects();
				
				// if(el.length > 0){
				// 	canvasPath = el[0].clipPath;
				// 	console.log(canvasPath)
				// }
				...
				noneMaskImg.onload = () => {
					...
					if (canvas4.width > canvas4.height) {
						...
						// console.log("-(outputImgH-outputImgW)/2=",-(outputImgH-outputImgW)/2)
						...
						// context3.drawImage(noneMaskImg, 10 * devicePixelRatio, 10 *devicePixelRatio, noneMaskImg.width/getScale - 20 * devicePixelRatio,noneMaskImg.height - 20 * devicePixelRatio, -(outputImgH -outputImgW) / 2, 0, outputImgW,outputImgH);
					...
					//-(outputImgH -outputImgW) / 2

					// console.log(canvas4.toDataURL());

					// $("body").append(" <div style='position:fixed;top:0;left:0'><img style='width:100vw;height:100vw' src='"+ canvas4.toDataURL() +"'>");

					// canvas.getActiveObject().clipPath = canvasPath;
					...
					// var file = base64ImgtoFile(base64); //有遮罩的效果图
					// var file2 = base64ImgtoFile(canvas4.toDataURL('image/jpg')); //无遮罩原始图片
					...
					// window.localStorage.removeItem('maskItemImg');
					// window.localStorage.removeItem('noMaskItemImg');
					// window.localStorage.clear();
					// window.localStorage.setItem('maskItemImg', base64); //设置数据
					// window.localStorage.setItem('noMaskItemImg', canvas4.toDataURL('image/png',0.8)); //设置数据
					...
					// console.log(base64);
					// console.log(canvas4.toDataURL('image/jpg'));

					return
					/******************** 测试用代码，正式发布注释掉 开始***************/
					// $("#xxxx").attr("src", "");
					// $("#xxxx").attr("src", canvas4.toDataURL('image/jpg'));
					// return
					/******************** 测试用代码，正式发布注释掉 结束***************/
					...
					/*
										...
										// document.getElementById("Canvas").height = windowWidth;
										...
										$.ajax({
											...
											success: function(data, status) {
												...
												if (data.code == 1) {
													...
													// window.location.href="index.html?id=" + getId + "&img=" + data.data.url;
													// $("#newPage").show();
													// $("#closePage").show();
													// $(".loadBox").hide();

													yl_img = data.data.url0; //保存预览的图片
													ys_img = data.data.url1; //保存原始的图片
													var pageUrl = "index.html?id=" + getId + "&img=" +
														yl_img + "&yimg=" + ys_img +
														'&token=' + token;
													// $('#newPage').load("index.html?id=" + getId + "&img=" + data.data.url);
...
$('#picker2').colpick(pickerConfig).keyup(function() {
